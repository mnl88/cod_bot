...
@dp.message_handler(chat_id=CHAT_TEST_ID, chat_type=['group', 'supergroup'], commands=['about', 'start'])
async def start_bot_not_in_the_main_group(message: types.Message):
    ...
    text = [
            ...
            group_link,
            '',
            'Для работы БОТА необходимо cообщить данные своей учётной записи в игре: ' + bot_link,
            '',
            'автор: @MaNiLe88'
    ]
    joined_text = '\n'.join(text)
...
@dp.message_handler(chat_type='private', commands=['about', 'start'])
